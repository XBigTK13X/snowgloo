...
export default class Search extends Component {
  ...
  render(){
    let artists = this.state.results && this.state.results.Artists.length ?
    (
      <div>
      <h3>{this.state.results.Artists.length} Artist{this.state.results.Artists.length > 1 ? 's':''}</h3>
    ...
    let albums = this.state.results && this.state.results.Albums.length ?
    (
      <div>
      <h3>{this.state.results.Albums.length} Album{this.state.results.Albums.length>1?'s':''}</h3>
      <div className="list-grid">
    ...
    let songs = this.state.results && this.state.results.Songs.length ?
    (
      <div>
    ...
    let results = this.state.searching ? 'Loading...' : this.state.results && this.state.results.ItemCount > 0?
      ...
      :
      (this.state.query?(<p>No results found for "{this.state.query}"</p>):null)
    return (
      <div>
